                            ...
                            <nav id="vertical-menu">
                                <ul class="main-menu1">
                                    @foreach ($menus as $category)
                                        ...
                                        @endphp
                                        <li <?php if(count($category->subcategories)>0){ ?>class="contain-submenu" <?php } ?> ><a href="{{route('category-products-list',['name' => $cat, 'id' => $category->id])}}">{{$category->name}}</a>
                                            @if($category->subcategories)
                                                <ul class="submenu-1">
                                                    @foreach ($category->subcategories as $subcategory)
                                                        <li <?php if(count($subcategory->subcategory1)>0){ ?> class="contain-submenu" <?php } ?> ><a href="{{route('subcategory-products-list',['id' => encrypt($subcategory->id)])}}">{{$subcategory->name}}</a>
                                                            @if ($subcategory->subcategory1)
                                                                <ul class="submenu-2">
                                                                    @foreach ($subcategory->subcategory1 as $sub1)
                                                                        <li <?php if(count($sub1->subcategory2)>0){ ?> class="contain-submenu" <?php } ?> ><a href="{{route('subcategory1-products-list',['id' => encrypt($sub1->id)])}}">{{$sub1->name}}</a>
