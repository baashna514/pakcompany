                    ...
                    <div class="category-derc">
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="banners">
                                    <div>
                                        <a  href="#"><img src="{{asset('public/front/image/slider2.webp')}}" alt="img cate" style="width: 100%;"><br></a>
                                    </div>
                                </div>
                                    ...
                                    <div class="product-item-container">
                                        <div class="left-block">
                                            <div class="product-image-container second_img">
                                                ...
                                                @endphp
                                                <a href="{{ route('product', ['id'=>($product->id)]) }}" target="_self" title="{{ $name }}">
                                                    <a href="{{ route('product', ['id'=>($product->id)]) }}" target="_self" title="{{ $product->name }}">
                                                        <img src="{{ asset('public/thumbnail/').'/'.$product->p_thumbnail }}" class="img-1 img-responsive" alt="{{ $product->name }}">
                                                        <img src="{{ asset('public/thumbnail/').'/'.$product->p_thumbnail }}" class="img-2 img-responsive" alt="{{ $product->name }}">
                                                    </a>
                                                </a>
                                            </div>
                                            <div class="box-label"> 
                                                @if ($product->p_type == 'simple product')
                                                    @php
                                                        $price = $product->price;
                                                    ...
                                                    @if ($product->sale_price)
                                                        <span class="label label-sale">-{{number_format((float)$perc, 0, '.', '') }}%</span>
                                                    @endif
                                                @endif
                                            </div>
                                            <div class="button-group so-quickview cartinfo--left">
                                                <a href="" data-id="{{$product->id}}" type="button" class="addToCart btn-button" title="Add to cart"><i class="fa fa-shopping-cart"></i><span>Add to Cart</span>
                                                </a>
                                                <button type="button" data-id="{{$product->id}}" class="wishlist btn-button" title="Add to Wish List"><i class="fa fa-heart"></i><span>Add to Wish List</span>
                                                </button>
                                                <!--quickview-->                                                      
                                        ...
                                        <div class="right-block">
                                            <div class="caption">
                                                ...
                                                <h4><a href="{{ route('product', ['id'=>($product->id)]) }}" title="{{ $product->name }}" target="_self">{{Str::limit($product->name, 30)}}</a></h4>
                                                @if ($product->sale_price)
                                                    <p class="price">   
                                                ...
                                                @else
                                                    <p class="price">   
                                                        <span class="price-new" id="price" data-id="{{$product->price}}">Rs. {{ $product->price }}</span>
                                                    </p>
                                                @endif
                                                ...
                                                <div class="list-block">
                                                    <button class="wishlist btn-button" type="button" title="Add to Wish List" onclick="wishlistfunction({{$product->id}})"><i class="fa fa-heart"></i>
                                                    </button>
                                                    <button class="compare btn-button" type="button" title="Compare this Product" onclick="compare.add('101');"><i class="fa fa-refresh"></i>
                                                    </button>
                                                    <!--quickview-->                                                      
                                                    <a class="iframe-link btn-button quickview quickview_handler visible-lg" href="{{ route('quick-view', ['id' => $product->id]) }}" title="Quick view" data-fancybox-type="iframe"><i class="fa fa-eye"></i></a>                                                        
                                                    <!--end quickview-->
                                                </div>
                                            </div>
                                        </div>
