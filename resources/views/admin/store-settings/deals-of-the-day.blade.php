                ...
                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
                    <div class="card">
                        <div class="header">
                            <h2>
                                Store Setting
                                <small>New Deal</small>
                            </h2>
                        </div>
                        <div class="body">
                            <form action="{{ route('md.store-setting.deals-of-the-day-action') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="row clearfix">
                                    <div class="col-sm-12">
                                            ...
                                            <div class="form-line">
                                                ...
                                                <select name="product_id" id="product_id" class="form-control">
                                                    ...
                                                    @foreach ($products as $product)
                                                        <option value="{{ $product->id }}" data-left="{{asset('public/thumbnail').'/'.$product->p_thumbnail}}" data-right="{{$product->price}}">{{ $product->name }} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; (Rs. <b class="price">{{$product->price}}</b>)</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                        ...
                        <div class="body">
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                                    ...
                                    <tbody>
                                        @foreach ($deals_of_the_day as $key=>$deal)
                                        @php
                                            $index = $key + 1;
                                        @endphp
                                        <tr>
                                            <td>{{ $index }}</td>
                                            <td>
                                                <img class="thumbnail" style="width: 100px;" src="{{ asset('public/thumbnail').'/'.$deal->product['p_thumbnail'] }}" alt="">
                                            </td>
                                            <td>{{ $deal->product['name'] }}</td>
                                            <td>{{ $deal->product['price'] }}</td>
                                            <td>{{ $deal->new_price }}</td>
                                            <td>
                                                <a href="{{ route('top-collection-delete', ['id' => $deal->id]) }}" class="btn btn-primary"><i class="material-icons">delete</i></a>
                                            </td>
                                        </tr>
                                        @endforeach
                                    </tbody>
